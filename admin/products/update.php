...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Handle image uploads (if new images are uploaded, replace the old ones)
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $conn->prepare("UPDATE products SET name=?, slug=?, category_id=?, category=?, price=?, status=?, description=?, ingredients=?, dosage=?, manufacturer=?, supplier_id=?, requires_prescription=?, max_quantity=?, sales=?, stock=?, how_to_use=?, precautions=?, image_01=?, image_02=?, image_03=? WHERE id=?");
            $stmt->execute([
                $formData['name'],
                $formData['slug'],
